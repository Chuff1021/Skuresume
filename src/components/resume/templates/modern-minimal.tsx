...
// Modern Minimal — single column, left-aligned, heavy whitespace.
// Pairing: Inter display + Source Serif 4 body (user can swap via metadata).

const BODY_FALLBACK = "'Source Serif 4', 'Source Serif Pro', Georgia, serif";
const DISPLAY_FALLBACK = "'Inter', 'Geist', system-ui, -apple-system, sans-serif";

export function ModernMinimalTemplate({ data }: { data: ResumeData }) {
  const { metadata } = data;
  ...
  return (
    <div
      className="resume-modern-minimal"
      style={{
        width: "100%",
        minHeight: "100%",
    ...
    >
      ...
      {data.summary.trim() ? (
        <Section>
          <p style={{ margin: 0, fontSize: t.body, lineHeight: t.lineBody, color: ink, whiteSpace: "pre-wrap" }}>
            {data.summary.trim()}
          </p>
        </Section>
      ) : null}
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        {contactBits.length ? (
          ...
          >
            {contactBits.map((c, i) => (
              <span key={i}>
                {i > 0 ? <span style={{ margin: "0 10px", color: withAlpha(ink, 0.25) }}>{"/"}</span> : null}
                {c}
              </span>
            ))}
          </div>
        ...
        {items(data.sections.projects).map((item) => (
          <div key={item.id} style={{ breakInside: "avoid" }}>
            ...
            {item.keywords?.length ? (
              <MetaRow left={item.keywords.join(" · ")} bodyFamily={t.bodyFamily} small={t.small} muted={muted} />
            ) : null}
            {item.description ? <ModernBullets text={item.description} size={t.body} lineHeight={t.lineBody} ink={ink} muted={muted} mark="em-dash" bodyFamily={t.bodyFamily} /> : null}
          </div>
        ))}
    ...
    <Section>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
        {items(data.sections.skills).map((item) => {
          ...
          return (
            <div key={item.id} style={{ fontFamily: t.bodyFamily, fontSize: t.body, lineHeight: t.lineBody, display: "flex", gap: 10 }}>
              {item.name ? (
                <span style={{ fontWeight: 600, color: ink, minWidth: 140, flexShrink: 0, letterSpacing: "-0.01em" }}>{item.name}</span>
              ) : null}
              <span style={{ flex: 1, color: ink }}>{kw.length ? kw.join(", ") : item.description}</span>
            </div>
          );
        })}
      </div>
...
function Languages({ data, ink, muted, t }: { data: ResumeData; ink: string; muted: string; t: ReturnType<typeof modernTypo> }) {
  ...
  return (
    <Section>
      ...
      <div style={{ fontFamily: t.bodyFamily, fontSize: t.body, lineHeight: t.lineBody, color: ink }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        {items(data.sections.references).map((item) => (
          <div key={item.id} style={{ fontFamily: t.bodyFamily, fontSize: t.body, lineHeight: t.lineBody }}>
            <div style={{ fontWeight: 600, letterSpacing: "-0.01em" }}>{item.name}</div>
