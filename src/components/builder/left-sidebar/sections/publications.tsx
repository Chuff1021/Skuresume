...
import { BookOpen, Plus, Trash, DotsSixVertical as GripVertical } from "@phosphor-icons/react";
...
export function PublicationsSection() {
  ...
  if (!data) return null;
  const { items } = data.sections.publications;

  const addItem = () => {
    updateData((d) => {
      d.sections.publications.items.push(createPublicationItem());
    });
  };

  const removeItem = (id: string) => {
    updateData((d) => {
      d.sections.publications.items = d.sections.publications.items.filter(
        (item) => item.id !== id
      );
    });
  };
  ...
  const updateItem = (id: string, field: string, value: string) => {
    updateData((d) => {
      const item = d.sections.publications.items.find((i) => i.id === id);
      if (item) {
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <Field label="Publisher">
                <Input
                  ...
                  onChange={(v) => updateItem(item.id, "publisher", v)}
              ...
              <Field label="URL">
                <Input
                  ...
                  onChange={(v) =>
                    updateData((d) => {
                      const i = d.sections.publications.items.find((x) => x.id === item.id);
                      if (i) i.url.url = v;
                    })
                  }
